...
IF EXISTS (SELECT 'X'
	...
	BEGIN
		...
		DECLARE C_PROV CURSOR FOR
			...
			BEGIN
				...
				OPEN C_PROV
				FETCH NEXT FROM C_PROV INTO @vID_PROVEEDOR, @vNOMBRE_MARCA, @vFECHA_INICIO
				WHILE @@FETCH_STATUS = 0
				BEGIN
					SET @vERROR_DATOS = 0
	...
	BEGIN
		DECLARE
			@vID_TIPO_MED		    VARCHAR(255), 
			@vID_PROVEEDOR		    VARCHAR(255), 
			@vID_PERSONA		    VARCHAR(255),
			@vID_FARMACIA		    VARCHAR(255),
			@vID_EMPLEADO		    VARCHAR(255),
			@vID_CLIENTE		    VARCHAR(255),
			@vID_MEDICAMENTO		VARCHAR(255),
			@vID_DISTRIBUIDOR		VARCHAR(255),
			@vCODIGO_RECETA_MEDICA  VARCHAR(255),
			@vID_PRECAUCION		    VARCHAR(255), 
			@vDURACION_ACUERDO	    VARCHAR(255),
			@vMESES_DURACION		VARCHAR(255),
			@vCAPACIDAD				VARCHAR(255),
			@vAÑOS_EXPERIENCIA		VARCHAR(255),
			@vCANTIDAD_ALMACEN		VARCHAR(255),
			@vCANTIDAD		        VARCHAR(255), 
			@FECHA_RECETA_MEDICA    VARCHAR(255),
			@vPrueba decimal(20,2),
			@vFechaEntero	integer
...
FROM FARMACIA_SA.dbo.MEDICAMENTO_SA ME INNER JOIN FARMACIA_SA.dbo.CATEGORIA_MEDICAMENTO_SA CME ON ME.ID_TIPO_MED = CME.ID_TIPO_MED 
			...
			BEGIN
				...
				OPEN C_FAC_FARM
				FETCH NEXT FROM C_PROV INTO @vID_TIPO_MED, @vID_PROVEEDOR, @vID_PERSONA, @vID_FARMACIA, @vID_EMPLEADO, @vID_CLIENTE, @vID_MEDICAMENTO, @vID_DISTRIBUIDOR, @vCODIGO_RECETA_MEDICA, @vID_PRECAUCION,  @vDURACION_ACUERDO, @vMESES_DURACION, @vCAPACIDAD, @vAÑOS_EXPERIENCIA, @vCANTIDAD_ALMACEN, @vCANTIDAD,  @FECHA_RECETA_MEDICA
				WHILE @@FETCH_STATUS = 0
				BEGIN
					SET @vERROR_DATOS = 0
					IF ISNUMERIC(@vID_TIPO_MED) = 0 OR @vID_TIPO_MED IS NULL
					...
					IF ISNUMERIC(@vDURACION_ACUERDO) = 0 OR @vDURACION_ACUERDO IS NULL
						SET @vERROR_DATOS = 1

					IF ISNUMERIC(@vMESES_DURACION) = 0 OR @vMESES_DURACION IS NULL
						SET @vERROR_DATOS = 1

					IF ISNUMERIC(@vCAPACIDAD) = 0 OR @vCAPACIDAD IS NULL
						SET @vERROR_DATOS = 1

					IF ISNUMERIC(@vAÑOS_EXPERIENCIA) = 0 OR @vAÑOS_EXPERIENCIA IS NULL
						SET @vERROR_DATOS = 1

					IF ISNUMERIC(@vCANTIDAD_ALMACEN) = 0 OR @vCANTIDAD_ALMACEN IS NULL
						SET @vERROR_DATOS = 1

					IF ISNUMERIC(@vCANTIDAD) = 0 OR @vCANTIDAD IS NULL
						SET @vERROR_DATOS = 1

					IF ISDATE(@FECHA_RECETA_MEDICA) = 0 OR @FECHA_RECETA_MEDICA IS NULL
						SET @vERROR_DATOS = 1
					ELSE 
						SET @vFechaEntero= CONVERT(INTEGER,REPLACE(@FECHA_RECETA_MEDICA,'-',''))

					IF @vERROR_DATOS= 1
						SET @VMENSAJE_ERROR= @VMENSAJE_ERROR + 'ERROR EN TRANSACCION: '+

			'Tipo medicamento: '+@vID_TIPO_MED+
			'Proveedor: '+@vID_PROVEEDOR+
			'Persona: '+ @vID_PERSONA+
			'Farmacia: '+@vID_FARMACIA+
			'Empleado: '+@vID_EMPLEADO+
			'Cliente: '+@vID_CLIENTE+
			'Medicamento: '+@vID_MEDICAMENTO+
			'Distribuidor: '+@vID_DISTRIBUIDOR+
			'Receta medica: '+@vCODIGO_RECETA_MEDICA+
			'Precaucion: '+@vID_PRECAUCION+
			'Duracion: '+@vDURACION_ACUERDO+
			'Meses de duracion: '+@vMESES_DURACION+
			'Capacidad: '+@vCAPACIDAD+
			'Años de experiencia: '+@vAÑOS_EXPERIENCIA+
			'Cantidad almacen: '+@vCANTIDAD_ALMACEN+
			'Cantidad: '+@vCANTIDAD+ 
			'Fecha: '+@FECHA_RECETA_MEDICA

					ELSE 
						INSERT INTO FAC_FARMACIA(ID_TIPO_MED, ID_PROVEEDOR, ID_PERSONA, ID_FARMACIA, ID_EMPLEADO, ID_CLIENTE, ID_MEDICAMENTO, ID_DISTRIBUIDOR, CODIGO_RECETA_MEDICA, ID_PRECAUCIÓN, DURACION_ACUERDO, MESES_DURACION, CAPACIDAD, AÑOS_EXPERIENCIA, CANTIDAD_ALMACEN, CANTIDAD, FECHA_RECETA_MEDICA) 
						VALUES(CONVERT (INTEGER(@vID_TIPO_MED), 
							   CONVERT (INTEGER(@vID_PROVEEDOR), 
							   CONVERT (INTEGER(@vID_PERSONA), 
							   CONVERT (INTEGER(@vID_FARMACIA), 
							   CONVERT (INTEGER(@vID_EMPLEADO), 
							   CONVERT (INTEGER(@vID_CLIENTE), 
							   CONVERT (INTEGER(@vID_MEDICAMENTO), 
							   CONVERT (INTEGER(@vID_DISTRIBUIDOR), 
							   CONVERT (INTEGER(@vCODIGO_RECETA_MEDICA), 
							   CONVERT (INTEGER(@vID_PRECAUCION),  
							   CONVERT (INTEGER(@vDURACION_ACUERDO), 
							   CONVERT (INTEGER(@vMESES_DURACION), 
							   CONVERT (INTEGER(@vCAPACIDAD), 
							   CONVERT (INTEGER(@vAÑOS_EXPERIENCIA), 
							   CONVERT (INTEGER(@vCANTIDAD_ALMACEN), 
							   CONVERT (INTEGER(@vCANTIDAD),  
							   @vFechaEntero))
					FETCH NEXT FROM C_FAC_FARM INTO @vID_TIPO_MED, @vID_PROVEEDOR, @vID_PERSONA, @vID_FARMACIA, @vID_EMPLEADO, @vID_CLIENTE, @vID_MEDICAMENTO, @vID_DISTRIBUIDOR, @vCODIGO_RECETA_MEDICA, @vID_PRECAUCION, @vDURACION_ACUERDO, @vMESES_DURACION, @vCAPACIDAD, @vAÑOS_EXPERIENCIA, @vCANTIDAD_ALMACEN, @vCANTIDAD, @vCANTIDAD
				END
				CLOSE C_FAC_FARM
